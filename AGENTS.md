...
| Layer | Technology |
|-------|------------|
| API | **Elixir / Phoenix** modular monolith (`kaarobar-backend`) |
...
- Duplicate anything that already lives in `shared/mobile/` (validations, RBAC-adjacent rules, i18n strings, money formatting, UI primitives) — fix it there so both mobile apps get it.
- Hand-edit `android/` or `ios/` in the Expo apps — those are generated by `expo prebuild` (CNG) and the edit is lost. Change `app.json` or add a config plugin.
