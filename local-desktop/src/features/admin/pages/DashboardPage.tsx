...
import { Banknote, Coins, PackageMinus, TrendingUp, Wallet } from 'lucide-react'
...
export function DashboardPage({ user, data }: Props) {
  ...
  const creditEnabled = hasLicenseFeature(useLicenseFeatures(), 'credit')
  ...
  return (
    <div>
      ...
      {summary ? (
        <>
          <Stagger className="mb-5 grid items-stretch gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            <KpiStat
            ...
            <KpiStat
              ...
              value={formatMoney(summary.profitTotal, currency)}
              meta={t('dashboard.kpiProfitDesc')}
              icon={<Coins className="size-5" aria-hidden />}
              tone={summary.profitTotal >= 0 ? 'success' : 'danger'}
            />
            {creditEnabled ? (
              <KpiStat
              ...
              <PaymentMixChart
                rows={
                  creditEnabled
                    ...
                    : summary.paymentsByMethod.filter((row) => row.method !== 'credit')
                }
                currency={currency}
              />
            </Card>
