...
export function getSalePrintLabels(
  lang: PrintLanguage = getPrintLanguage(),
): SalePrintLabels {
  return SALE_LABELS[lang];
}

export function getPoPrintLabels(
  lang: PrintLanguage = getPrintLanguage(),
): PoPrintLabels {
  return PO_LABELS[lang];
}

export function getLedgerPrintLabels(
  lang: PrintLanguage = getPrintLanguage(),
): LedgerPrintLabels {
...
export function getPrintPreviewLabels(
  lang: PrintLanguage = getPrintLanguage(),
): PrintPreviewLabels {
 ...
 * are loaded in hidden windows whose `did-finish-load` waits on stylesheets —
 * a Google Fonts <link> on a machine without internet can stall every receipt
 * for the length of a network timeout, and the font never arrives anyway. The
 * stacks below use the app's fonts when installed and fall back to system
 * faces that cover Latin and Arabic-script text.
 */
