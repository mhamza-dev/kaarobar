  ...
  nil ->
    ...
    :ok =
      Repo.with_tenant_context(result.organization.id, fn ->
        ...
        # to exercise it with.
        grocery =
          scope |> Tenancy.list_businesses() |> Enum.find(&(&1.business_type == "grocery"))
        ...
        suppliers =
          for supplier <- [
            ...
            {:ok, supplier} = Kaarobar.Purchasing.create_supplier(shop, supplier)
            supplier
          end
        ...
        # --- Stock paperwork ------------------------------------------------------
        #
        # One of each document in the state a screen most needs showing: a
        # transfer that has made the whole trip, a count waiting on a manager
        # with a real variance in it, and a purchase order that has been
        # delivered. Empty lists prove nothing about a detail page.

        variant = &Map.fetch!(variants_by_name, &1).id

        {:ok, transfer} =
          Kaarobar.Inventory.create_transfer(shop, %{
            "source_branch_id" => shop.branch.id,
            "destination_branch_id" => warehouse.id,
            "notes" => "Overstock to the godown",
            "items" => [
              %{"variant_id" => variant.("Basmati rice 25kg sack"), "quantity" => "4"},
              %{"variant_id" => variant.("Cooking oil 5L tin"), "quantity" => "10"}
            ]
          })

        {:ok, transfer} = Kaarobar.Inventory.dispatch_transfer(shop, transfer)
        {:ok, transfer} = Kaarobar.Inventory.receive_transfer(shop, transfer)

        {:ok, count} =
          Kaarobar.Inventory.create_count(shop, %{
            "branch_id" => shop.branch.id,
            "kind" => "cycle",
            "category_id" => categories["Rice & grains"].id,
            "notes" => "Monthly rice shelf count"
          })

        # Everything tallies except two short bags of sella: the variance the
        # approval screen is there to catch.
        sella = variant.("Sella rice 5kg")

        for item <- count.items do
          counted =
            if item.variant_id == sella,
              do: Decimal.sub(item.expected_quantity, 2),
          ...
          attrs =
            if item.variant_id == sella,
              ...
              else: %{"counted_quantity" => counted}

          {:ok, _item} = Kaarobar.Inventory.record_count(shop, item, attrs)
        end

        {:ok, count} = Kaarobar.Inventory.fetch_count(shop, count.id)
        {:ok, count} = Kaarobar.Inventory.submit_count(shop, count)

        al_madina = Enum.find(suppliers, &(&1.name == "Al-Madina Wholesale"))

        {:ok, order} =
          Kaarobar.Purchasing.create_order(shop, %{
            "supplier_id" => al_madina.id,
            "branch_id" => shop.branch.id,
            "items" => [
              %{
                "variant_id" => variant.("Basmati rice 5kg"),
                "ordered_quantity" => "20",
                "unit_cost" => "1600.00"
              },
              %{
                "variant_id" => variant.("Cooking oil 1L"),
                "ordered_quantity" => "48",
                "unit_cost" => "540.00"
              }
            ]
        ...
        {:ok, order} = Kaarobar.Purchasing.approve_order(shop, order)
        {:ok, order} = Kaarobar.Purchasing.fetch_order(shop, order.id)

        {:ok, receipt} =
          Kaarobar.Purchasing.create_receipt(shop, %{
            "supplier_id" => al_madina.id,
            "branch_id" => shop.branch.id,
            "purchase_order_id" => order.id,
            "supplier_reference" => "AMW-20417",
            "items" =>
        ...
        {:ok, receipt} = Kaarobar.Purchasing.post_receipt(shop, receipt)

        Logger.info(
          "  transfer #{transfer.number} received, count #{count.number} awaiting approval, " <>
            "#{order.number} delivered on #{receipt.number}"
        )
        ...
        # A kitchen needs somewhere to send orders before anything can be fired.
        {:ok, restaurant} =
          Scopes.build(result.user, %{business_id: businesses_by_type["restaurant"].id})

        for name <- ["Tandoor", "Karahi line"] do
          {:ok, _station} = Kaarobar.Kitchen.create_station(restaurant, %{"name" => name})
        end
        ...
        Demo data ready.
          ...
          completed sale. A received transfer, a rice-shelf count awaiting approval
          and a delivered purchase order. Two invitations are still pending, and
          "Weekend cashier" is a custom role.

          Karahi Corner: two kitchen stations.
        """)
