...
function StaffAccountingPage() {
  ...
  const [tab, setTab] = useTabQueryParam<Tab>("tb", ACCOUNTING_TABS, {
    basePath: routes.accounting,
  });
  ...
  const [coaFilters, setCoaFilters] = useState(emptyStaffListFilters);
    ...
    >
      ...
      {tab === "coa" ? (
        <DataTable
          ...
          filterState={coaFilters}
          onFilterChange={setCoaFilters}
          clientFilter
          filterAccessors={{
            searchText: (row) =>
              `${row.account.code} ${row.account.name} ${row.account.type} ${row.account.classification || ""}`,
          }}
          filterConfig={{}}
          ...
          pagination={{ mode: "client", pageSize: 40 }}
          exportable
          ...
          getExportRow={(row) => ({
            ...
            normal_balance: row.account.normal_balance || "",
            header: row.account.is_header ? "yes" : "no",
          })}
          exportColumns={[
      ...
      {tab === "ar" ? (
        <div className="space-y-3">
          ...
          <DataTable
            maxHeight="28rem"
            loading={arLoading}
            filterState={arFilters}
            onFilterChange={setArFilters}
            filterAccessors={{
              searchText: (r) =>
      ...
      {tab === "ap" ? (
        <DataTable
          maxHeight="28rem"
          loading={apLoading}
          filterState={apFilters}
          onFilterChange={setApFilters}
          filterAccessors={{
            searchText: (r) =>
