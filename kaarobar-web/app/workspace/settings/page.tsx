...
export default function SettingsPage() {
  ...
  async function saveMarketplace(b: Business) {
    try {
      await api(`/businesses/${b.id}`, {
        method: "PATCH",
        body: JSON.stringify({
          marketplace_enabled: !!b.marketplace_enabled,
          marketplace_slug: b.marketplace_slug || null,
          online_branch_id: b.online_branch_id || session?.branch_id || null,
        }),
      });
    ...
    <div className="space-y-6">
      ...
      {tab === "integrations" && isOwner ? (
        <div className="space-y-4">
          ...
          <SurfaceCard className="space-y-3 p-5">
            <h2 className="font-semibold text-heading">Marketplace</h2>
            <p className="text-sm text-body">
              List this business on the customer marketplace so shoppers can browse and place
              pickup orders online.
            </p>
            {businesses.map((b) => (
              <div
                key={`market-${b.id}`}
              ...
              >
                <span className="font-medium text-heading sm:col-span-3">{b.name}</span>
                <label className="flex items-center gap-2 text-sm text-body">
                  <input
                    type="checkbox"
                    checked={!!b.marketplace_enabled}
                    onChange={(e) =>
                      setBusinesses((prev) =>
                        prev.map((x) =>
                          x.id === b.id
                            ? { ...x, marketplace_enabled: e.target.checked }
                            : x
                        )
                      )
                    }
                ...
                <label className="text-xs text-body">
                  ...
                  <input
                    className="mt-1 w-full rounded border border-border px-2 py-1 text-sm"
                    value={b.marketplace_slug || ""}
                    placeholder="my-cafe"
                    onChange={(e) =>
                      setBusinesses((prev) =>
                        prev.map((x) =>
                          x.id === b.id ? { ...x, marketplace_slug: e.target.value } : x
                        )
                      )
                    }
                  />
                ...
                <button
                  type="button"
                  onClick={() => void saveMarketplace(b)}
                  className="self-end rounded-md bg-brand px-3 py-2 text-sm font-semibold text-brand-foreground"
                >
                  {t("common.save")}
                </button>
