...
/** Re-checks the signed-in user's password — used by the screen-lock unlock. */
export function verifySessionPassword(password: string): { ok: boolean } {
  ...
  try {
    ...
    const row = getDb()
      ...
      .get(session.id) as { password_hash: string; is_active: number } | undefined
