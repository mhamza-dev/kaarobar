...
/**
 * Connections that already ran the migration sweep. The domain layer calls
 * runMigrations on every db() access; re-checking a dozen migration rows each
 * time is pure overhead. A WeakSet (not a boolean) so a fresh connection after
 * a backup restore migrates again.
 */
