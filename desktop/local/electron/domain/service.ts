...
import { resolveDefaultPrinterName } from "../receipt/printerDevices";
import { sendRawToPrinter } from "../receipt/rawPrint";
...
function mapProductRow(row: {
  ...
  return {
    ...
    kind,
    tracksStock:
      row.tracks_stock == null
        ? defaultTracksStock(kind)
        : Boolean(row.tracks_stock),
...
export function createBusiness(payload: {
  ...
  const existing = db().prepare("SELECT id FROM businesses LIMIT 1").get() as
    | { id: string }
    | undefined;
  if (existing) {
    throw new Error(
...
export function updateBranch(payload: {
  ...
  const existing = db()
    ...
    .get(payload.id) as
    | { business_id: string; is_main_branch: number; is_active: number }
    | undefined;
...
/** Blocks adding/reactivating staff past the license plan's seat count. */
function assertUserSeatAvailable(businessId: string): void {
...
export function setUserActive(payload: { userId: string; isActive: boolean }): {
  ok: true;
} {
  requirePermission("users:manage");
  const row = db()
    .prepare("SELECT business_id FROM users WHERE id = ?")
    .get(payload.userId) as { business_id: string } | undefined;
  ...
  db()
    ...
    .run(payload.isActive ? 1 : 0, payload.userId);
  return { ok: true };
}
...
export function updateProduct(payload: {
  ...
  const kind: ProductKind =
    payload.kind ?? ((existing.kind as ProductKind) || "item");
  if (!isValidProductKind(nature, kind)) {
    throw new Error(
      `Product kind "${kind}" is not allowed for this business type`,
    );
...
export function setProductActive(payload: { id: string; isActive: boolean }): {
  ok: true;
} {
  requireValidLicense();
  const session = requirePermission("products:edit");
  ...
  db()
    ...
    .run(payload.isActive ? 1 : 0, payload.id);
  writeActivity({
    businessId: existing.business_id,
    actorUserId: session.id,
    entityType: "product",
...
export function deleteProduct(id: string): {
  ...
  const existing = db()
    ...
    .get(id) as
    | { business_id: string; name: string; is_active: number }
    | undefined;
  if (!existing) throw new Error("Product not found");
  assertBusinessAccess(existing.business_id);
  ...
  const inSales = db()
    .prepare("SELECT id FROM sale_items WHERE product_id = ? LIMIT 1")
    .get(id);
  const inPo = db()
    .prepare("SELECT id FROM purchase_order_items WHERE product_id = ? LIMIT 1")
    .get(id);
  if (inSales || inPo) {
...
export function generateProductBarcode(businessId: string): {
  barcode: string;
} {
  requireValidLicense();
  requirePermission("products:edit");
...
export function listSuppliers(businessId: string): Supplier[] {
  assertBusinessAccess(businessId);
  // Gated plans get an empty list, not an error — useAdminData loads all
  // modules in one Promise.all and one rejection would blank every page.
  if (!isFeatureEnabled("suppliers")) return [];
  const rows = db()
    .prepare(
...
function requireSupplier(supplierId: string): {
  id: string;
  business_id: string;
  name: string;
  phone: string | null;
...
export function unlinkSupplierProduct(payload: {
  supplierId: string;
  productId: string;
}): { ok: true } {
  requireFeature("suppliers");
...
export function createCustomer(payload: {
  ...
  const id = uuidv4();
  const at = nowIso();
  // Opening balance is a credit-book concept; plans without credit start at 0.
  const startingAmount = isFeatureEnabled("credit")
    ? Math.max(0, Number(payload.amount ?? 0) || 0)
    : 0;
  const cleanAddress = payload.address?.trim() || null;
...
export function updateCustomer(payload: {
  ...
  const existing = db()
    ...
    .get(payload.id) as
    | { business_id: string; current_balance: number }
    | undefined;
...
export function getCustomerDetail(customerId: string): CustomerDetail {
  ...
  return {
    ...
    sales: sales.map((sale) => {
      const methods = paymentStmt.all(sale.id) as Array<{
...
export function createSale(payload: {
  ...
  if (showsServedBy(nature)) {
    ...
    const staff = db()
      ...
      .get(servedByUserId, payload.businessId, payload.businessId) as
      | { id: string }
      | undefined;
  ...
  if (showsServiceMode(nature)) {
    if (
      !serviceMode ||
      !["dine_in", "takeaway", "delivery"].includes(serviceMode)
    ) {
  ...
  if (serviceMode === "takeaway" || serviceMode === "delivery") {
    if (riderUserId) {
      const rider = db()
        ...
        .get(riderUserId, payload.businessId, payload.businessId) as
        | { id: string }
        | undefined;
  ...
  db().transaction(() => {
    ...
    if (payload.customerId && creditAmount > 0) {
      ...
      db()
        ...
        .run(newBalance, payload.customerId);
      db()
        .prepare(
          `INSERT INTO ledger_entries (id, customer_id, business_id, branch_id, type, amount, balance_after, reference_sale_id, note, created_by, created_at)
    ...
    if (ticketId) {
      if (partialTicketBill) {
        ...
        if (remaining.c === 0) {
          ...
          db()
            .prepare(`UPDATE pos_tickets SET updated_at = ? WHERE id = ?`)
            .run(at, ticketId);
        }
      } else {
        db()
...
export function voidRefundSale(payload: { saleId: string; reason: string }): {
  ok: true;
} {
...
export function createRefundRequest(payload: {
  ...
  const sale = db()
    ...
    | { id: string; business_id: string; status: Sale["status"] }
    | undefined;
...
export function findSaleByInvoice(
  businessId: string,
  invoiceNo: string,
): Sale | null {
  requireValidLicense();
...
export function updateSaleDelivery(payload: {
  ...
  if (riderUserId) {
    const rider = db()
      ...
      .get(riderUserId, sale.businessId, sale.businessId) as
      | { id: string }
      | undefined;
    if (!rider) throw new Error("Rider not found");
...
export async function printSaleReceipt(saleId: string): Promise<SalePrintResult> {
  ...
  // POS printing switched off: the pre-existing behaviour — a visible preview
  // window the cashier prints from — is what the setting promises.
  ...
  try {
    // Raw ESC/POS: the transport for thermal printers on raw/passthrough
    // queues, where a rendered page arrives as PostScript source and prints
    // literally. Sheet paper (ink/laser) always renders through the driver —
    // ESC/POS bytes mean nothing to an office printer.
    if (effectiveTransport(settings) === "raw") {
      // The spooler APIs need a concrete queue name — there is no "OS default"
      // for a raw job — so resolve one when none is configured. Prefer a
      // thermal-looking queue: ESC/POS bytes sent to the office inkjet that
      // happens to be the machine default would print nothing useful.
      const printerName =
        settings.posPrinterName ||
        (await resolveDefaultPrinterName({ preferThermal: true }));
      if (!printerName) {
        throw new Error(
    ...
    // ordinary page, which works on any printer with a real driver — thermal
    // or ink/laser — on all three platforms. Roll sizes get the narrow layout,
    // A4/Letter get the full-page one.
    const html = await buildSaleReceiptHtml(receiptInput, { paper, template });
    const outcome = await printHtmlSilent({
      html,
      printerName:
        settings.posPrinterName || (await resolveDefaultPrinterName()),
      copies: settings.posCopies,
      silent: settings.posSilent,
      pageSize: isRollPaper(paper) ? undefined : (paper as "A4" | "Letter"),
    });
    return {
      ok: true,
    ...
    // A till that prints nothing is worse than one that opens a window: keep
    // the receipt on screen so it can be reprinted or read to the customer,
    // and surface why the printer path failed.
    await showPreview();
    return {
      ok: true,
      method: "preview",
...
export function getAnalyticsSummary(payload: {
  ...
  const salesRows = db()
    ...
    .all(businessId, sinceIso, untilIso) as Array<{
    day: string;
    total: number;
    count: number;
  }>;
  ...
  // Estimated profit: per-line revenue (line_total already nets line
  // discounts) pro-rated by the un-refunded quantity, minus the cost of those
  // units. Products with no cost_price count as zero cost, and the bill-level
  // discount is subtracted afterwards.
  const profitRow = db()
    .prepare(
      `SELECT COALESCE(SUM(
...
export function openPosTicket(payload: {
  ...
  const deliveryStatus =
    payload.serviceMode === "takeaway" || payload.serviceMode === "delivery"
      ? "pending"
      : null;
  db()
    .prepare(
...
export function setPosTicketItems(payload: {
  ...
  db().transaction(() => {
    ...
    db()
      .prepare("DELETE FROM pos_ticket_items WHERE ticket_id = ?")
...
export function fireTicketItems(payload: {
  ticketId: string;
  itemIds: string[];
}): PosTicket {
  requireValidLicense();
...
export function assignTicketRider(payload: {
  ...
  if (riderUserId) {
    const rider = db()
      ...
      .get(riderUserId, ticket.businessId, ticket.businessId) as
      | { id: string }
      | undefined;
    if (!rider) throw new Error("Rider not found");
    if (!deliveryStatus || deliveryStatus === "pending")
...
export function recallKitchenItems(payload: { itemIds: string[] }): {
  ok: true;
} {
  requireValidLicense();
  requirePermission("sales:checkout");
...
function assertHappyHourPricing(payload: {
  ...
  if (
    !Number.isInteger(payload.weekdaysMask) ||
    payload.weekdaysMask < 0 ||
    payload.weekdaysMask > 127
  ) {
...
function localParts(atIso?: string): {
  date: string;
  weekdayBit: number;
  hm: string;
} {
...
export function resolveUnitPrice(payload: {
  ...
  const product = db()
    ...
    | undefined;
  if (!product) throw new Error("Product not found");
  // Plans without happy hour sell at list price — rules are never applied.
...
export function setHappyHourRuleActive(payload: {
  id: string;
  isActive: boolean;
}): HappyHourPriceRule {
  requireValidLicense();
