...
export function SupplierDetailPage({
  ...
  const [unlinkTarget, setUnlinkTarget] = useState<SupplierProduct | null>(
    null,
  );
  const [deleteTarget, setDeleteTarget] = useState<SupplierProduct | null>(
    null,
  );
  const {
    products,
        ...
        try {
          ...
          toast.success(
            row.product.isActive
              ? t("toast.productDeactivated")
              : t("toast.productActivated"),
          );
        } catch (e) {
  ...
  return (
    <div>
        ...
        description={
          loading
            ...
            : [supplier?.phone, supplier?.address]
                .filter(Boolean)
                .join(" · ") || t("dashboard.supplierDetailDesc")
        }
        actions={
        ...
        <div className="mb-6 overflow-hidden rounded-lg border border-brand-primary/15 bg-gradient-to-br from-brand-tint/70 via-surface-raised to-surface-raised p-5 shadow-soft">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              {supplier.notes ? (
                <p className="mt-2 text-sm text-ink-muted">{supplier.notes}</p>
              ) : null}
            </div>
      ...
      >
        {!detail || detail.products.length === 0 ? (
          ...
          <Table
            ...
            search={{
              getText: (row) =>
                `${row.product.name} ${row.product.barcode ?? ""}`,
            }}
            filters={[
              {
                id: "active",
            ...
            mobileCardActions={(row) => (
              <RowActionsMenu actions={supplierProductActions(row)} />
            )}
            columns={[
              {
                key: "name",
      ...
      >
        <Formik
          ...
          initialValues={{
            ...
            unitCost:
              products.find((p) => p.id === attachableProducts[0]?.value)
                ?.costPrice ?? 0,
          }}
          ...
          onSubmit={async (values) => {
            try {
              ...
              toast.success(
                t("toast.productAttached", { name: product?.name ?? "" }),
              );
              await load();
              setAttachOpen(false);
            } catch (e) {
            ...
            <Form className="space-y-3">
              ...
              <div className="flex flex-col-reverse justify-end gap-2 sm:flex-row">
                ...
                <Button
                  ...
                  disabled={attachableProducts.length === 0}
                >
                  {t("forms.attachProduct")}
                </Button>
              </div>
        ...
        onSubmit={async (values) => {
          ...
          try {
            const product = await window.api.products.create({
              ...
              costPrice:
                values.costPrice == null ? undefined : Number(values.costPrice),
            ...
            toast.success(
              t("toast.productCreatedAndAttached", { name: product.name }),
            );
      ...
      <ConfirmDialog
        ...
        description={t("forms.deleteProductConfirm", {
        ...
        onConfirm={async () => {
          ...
          try {
            const result = await window.api.products.delete(
