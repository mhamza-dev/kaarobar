...
export function UsersPage({ user, data }: Props) {
  ...
  const seatLimitReached =
    Number.isFinite(maxUsers) && activeStaffCount >= maxUsers;
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <Button
                        ...
                        className="w-full"
                        onClick={async () => {
                          const picked = await window.api.assets.pickAndSave({
                            kind: "product",
                          });
                          if (picked)
                            setFieldValue("imagePath", picked.relativePath);
                        }}
                      >
                        {t("forms.chooseImage")}
                      </Button>
      ...
      <Card title={t("dashboard.staffUsers")}>
        {staff.length === 0 ? (
          <EmptyState
            ...
            description={t("empty.noUsersDesc")}
          />
        ) : (
          <Table
            embedded
