...
export function ProductsPage({ user, data }: Props) {
  ...
  const lowStock = products.filter(
    (p) => p.isActive && p.tracksStock && p.stockQty <= 5,
  ).length;
  ...
  return (
    <div>
        ...
        <div className="mb-5 grid gap-3 sm:grid-cols-3">
          <div className="rounded-lg border border-line bg-surface-raised px-4 py-3 shadow-soft">
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-subtle">
              {t("dashboard.statTotal", { count: products.length })}
            </p>
          </div>
          <div className="rounded-lg border border-line bg-surface-raised px-4 py-3 shadow-soft">
            <p className="text-xs font-semibold uppercase tracking-wide text-warning">
              {t("dashboard.statLowStock", { count: lowStock })}
            </p>
          </div>
          <div className="rounded-lg border border-line bg-surface-raised px-4 py-3 shadow-soft">
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-muted">
              {t("dashboard.statInactive", { count: inactive })}
            </p>
          </div>
        </div>
      ) : null}
      ...
      <Card
        title={t("dashboard.productCatalog")}
        description={t("dashboard.productsDesc")}
      >
        {products.length === 0 ? (
          <EmptyState
          ...
          <Table
            ...
            mobileCardFields={[
              ...
              },
              {
                key: "stock",
                label: t("forms.stock"),
                render: (row) => (row.tracksStock ? row.stockQty : "—"),
              },
              {
                key: "status",
                label: t("forms.status"),
            ...
            mobileCardActions={(row) => (
              <RowActionsMenu actions={productActions(row)} />
            )}
            columns={[
              {
                key: "name",
          ...
          try {
            if (selected && mode === "edit") {
              await window.api.products.update({
                ...
                costPrice:
                  values.costPrice == null ? null : Number(values.costPrice),
