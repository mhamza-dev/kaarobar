  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        description={
          detail
            ? [detail.customer.phone, t("forms.credit")]
                .filter(Boolean)
                .join(" · ")
      ...
      {detail ? (
        <div className="space-y-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            {creditEnabled ? (
              <Card title={t("forms.paymentsReceived")} accent="success">
                <p className="text-xl font-bold text-ink">
                  {formatMoney(summary.paymentsReceived)}
                </p>
              </Card>
            ) : null}
          </div>
          ...
          <Card
            title={t("forms.purchaseHistory")}
            description={t("forms.purchaseHistoryDesc")}
          >
            {detail.sales.length === 0 ? (
              <EmptyState
                title={t("empty.noSales")}
                description={t("empty.noSalesDesc")}
              />
            ) : (
              <Table
                embedded
                ...
                filters={[
                  {
                    ...
                    options: [
                      {
                        value: "completed",
                        label: statusLabel(t, "sale", "completed"),
                      },
                      { value: "void", label: statusLabel(t, "sale", "void") },
                      {
                        value: "refunded",
                        label: statusLabel(t, "sale", "refunded"),
                      },
                      {
                  ...
                  },
                  {
                    ...
                    render: (row) =>
                      row.paymentMethods.length
                        ? row.paymentMethods
                            .map((m) =>
                              t(paymentMethodI18nKey(m), { defaultValue: m }),
                            )
                            .join(", ")
                        : "—",
                  },
