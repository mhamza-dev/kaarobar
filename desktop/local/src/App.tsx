...
export default function App() {
  ...
  const licenseLoaded = useLicenseStore((state) => state.loaded)

  // Load the license plan before the dashboard mounts so nav gating never
  // flashes routes the plan does not include. AppShell keeps it fresh after.
  useEffect(() => {
    if (!user || licenseLoaded) return
    let cancelled = false
    ;(async () => {
      try {
        const summary = await window.api.app.getLicenseStatus()
        if (!cancelled) useLicenseStore.getState().setLicense(summary)
      } catch {
        // Pessimistic fallback: treat as legacy/unrestricted; guards in the
        // main process still enforce the real plan.
        if (!cancelled) {
