...
import { canUseNavRoute, type LicenseFeature } from './license'
...
export function getVisibleNavItems(
  ...
  licenseFeatures?: LicenseFeature[] | null,
): NavItem[] {
  ...
  return visibleNavRoutes(user)
    ...
    .filter((id) => canUseNavRoute(licenseFeatures, id))
    .map((id) => ({
      id,
...
export function defaultNavRoute(
  ...
  const routes = getVisibleNavItems(user, businessNature, licenseFeatures).map((item) => item.id)
...
export function resolveNavRoute(
  ...
  if ((requested === 'tables' || requested === 'kitchen') && !showsTables(nature)) {
    return defaultNavRoute(user, nature, licenseFeatures)
  }
  if (!canUseNavRoute(licenseFeatures, requested as NavRouteId)) {
    return defaultNavRoute(user, nature, licenseFeatures)
  }
...
export function useActionVisibility(user: SessionUser | null) {
  return {
    ...
    canDeleteCustomers: can(user, 'customers:delete'),
    canDeleteSuppliers: can(user, 'suppliers:delete'),
    canDeletePurchaseOrders: can(user, 'purchaseOrders:delete'),
