...
export function AppShell({
  ...
  const navItems = getVisibleNavItems(user, businessNature, licenseFeatures).filter(
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        // Keep the shared plan/feature mirror fresh (mid-session upgrades).
        useLicenseStore.getState().setLicense(status)
