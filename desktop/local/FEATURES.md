# Kaarobar POS — Features & Benefits

How Kaarobar helps a single shop sell, track stock, manage credit, and stay online-ready without depending on the internet for day-to-day work.

**Features appear by business type.** At setup you choose retail, food, salon, or services — tables, service modes, staff “served by,” and product kinds only show when they apply to that shop.
...
- Supports **English, Urdu, German, Portuguese, Spanish, French, and Arabic** (Urdu & Arabic with RTL layout)  
- Keeps **cash, card/online, and credit** sales in one place  
...
| Cash, **Card / Online**, or credit payment | Match how shops get paid |
| Overstock highlight + block for stock-tracked products | Prevent selling more than you have |
| Print / reprint receipts (layout follows the shop language) | Customer proof in the language the shop uses |
...
| What you get | How it helps |
|---|---|
| KPIs (sales, payments, credit outstanding, low stock) | See shop health at a glance |
...
| Large amounts in **Lakh / Crore** for **PKR** shops | Easier reading for South Asian totals; other currencies use locale formatting |
...
| Printable POs (shop language layout) | Share clear orders with suppliers |

**Outcome:** Purchasing is documented instead of living only in notebooks or WhatsApp chats.

Suppliers and purchase orders appear as separate sidebar items.
...
| Classic debit / credit ledger with date filter | Read credit like a paper book |
| Print ledger (selected dates, or full if none) | Share or archive customer statements |
| Credit (pay later) sales with balance updates | Serve regulars safely without losing track of credit |
...
| **Single latest file** — create/auto-backup writes `Documents/KaarobarBackups/kaarobar-latest.kaarobar-backup` and replaces any previous backup in that folder | No pile-up of timestamped copies; restore always has one clear latest |
...
| What you get | How it helps |
|---|---|
| Full UI in EN, UR, DE, PT, ES, FR, AR | Staff use the language they prefer |
| Urdu and Arabic with right-to-left layout | Comfortable reading and navigation |
| **Poppins** (LTR) and **Noto Sans Arabic** (UR/AR) | Clear typography for dense POS screens |
| Receipts & PO prints follow the selected language (font, RTL, labels) | Printed paper matches the shop language |
...
1. **Morning open** — Cashier logs in → POS → scan items → take cash / card-online / credit → print receipt.  
...
4. **Owner check-in** — Dashboard → pick 7 or 30 days → review top products and credit outstanding.  
...
7. **Safety net** — Enable daily auto-backup (or Create backup now) → updates the latest file in Documents/KaarobarBackups (replaces the previous) → keep a copy off the shop PC.  
8. **Reminders** — Sign in → open header **Reminders** for restock / license notes.

---

## What Kaarobar is *not* (current stage)
...
| Doc | Purpose |
|---|---|
| [README.md](./README.md) | Setup, develop, build installers |
| [PHASE_GATES.md](./PHASE_GATES.md) | Pre-release product checklist |
| [WINDOWS_RELEASE_SMOKE_TEST.md](./WINDOWS_RELEASE_SMOKE_TEST.md) | Windows install smoke test |

---

*Document reflects the current single-shop, business-nature-aware product stage of Kaarobar POS.*
