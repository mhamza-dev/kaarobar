...
Status against KRB-SRS-004 **v4.1** Must items for reporting, billing, Pakistan FBR fiscal pack, notifications, and **Cloud Desktop** offline sync (`OFF-FR`).
...
| ADM-FR-003 | Done | **Safepay** HMAC webhook + hosted checkout (`POST /billing/checkout`); env fallback URL (Pakistan Cloud path: JazzCash / Easypaisa / cards). Other-market providers Should (SRS FUT-FR-026). |
| ADM-FR-005 | Partial | Expired trial / cancelled / paused / past_due block creates (`subscription_inactive`) |
| FBR-FR-001 | Done | `businesses.fbr_tier1` flag (Settings UI); **Growth+** plan required; Pakistan fiscal pack only |
...
| FBR-FR-007 | Partial | Adapter is async/isolated; formal multi-jurisdiction fiscal-pack interface still evolving |
...
| OFF-FR-006 | Done | FBR still async after online/sync sale (when pack enabled) |

Deferred: real FBR production adapter, full customer billing portal (ADM-FR-004), UK/DE/FR fiscal packs (FUT-FR-023). Lemon Squeezy is **retired** from the live path.
