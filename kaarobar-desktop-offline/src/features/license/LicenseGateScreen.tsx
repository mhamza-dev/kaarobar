...
const SUPPORT_EMAIL = 'mhamzadev.contact@gmail.com'
...
export function LicenseGateScreen({
  ...
  return (
    ...
    >
      <Card
        title={mode === 'expired' ? t('license.expiredTitle') : t('license.missingTitle')}
        description={mode === 'expired' ? t('license.expiredDesc') : t('license.missingDesc')}
        accent="brand"
      >
        <div className="mb-4 space-y-2 text-sm text-ink-muted">
          {issuedTo ? (
            <p>
              <span className="font-medium text-ink">{t('license.issuedTo')}: </span>
              {issuedTo}
            </p>
          ) : null}
          {mode === 'expired' && expiryLabel ? (
            <p>
              <span className="font-medium text-ink">{t('license.expiredOn')}: </span>
              {expiryLabel}
            </p>
          ) : null}
