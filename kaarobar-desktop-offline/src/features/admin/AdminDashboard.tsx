...
export function AdminDashboard({ user, onLogout, onLicenseLocked }: Props) {
  ...
  const [route, setRoute] = useState<NavRouteId>(() => defaultNavRoute(user, businessNature))
  ...
  useEffect(() => {
    setRoute((current) => resolveNavRoute(user, current, businessNature))
  }, [user, businessNature])
  ...
  function changeRoute(next: NavRouteId) {
    ...
    setPoDetailId(null)
    setRoute(resolveNavRoute(user, next, businessNature))
  }
