    ...
    <div>
      ...
      {customers.length > 0 ? (
        <div className="mb-5 rounded-lg border border-warning/20 bg-gradient-to-br from-warning-soft/40 to-surface-raised px-4 py-3 shadow-soft sm:px-5">
          ...
          <p className="mt-1 text-xl font-bold text-ink">{formatMoney(creditOutstanding)}</p>
      ...
      <Card title={t('dashboard.customers')} description={t('dashboard.customersDesc')}>
        {customers.length === 0 ? (
          <EmptyState title={t('empty.noCustomers')} description={t('empty.noCustomersDesc')} />
        ) : (
          <Table
            embedded
            pageSize={10}
            ...
            mobileCardActions={(row) => <RowActionsMenu actions={customerActions(row)} />}
            columns={[
              {
                key: 'name',
                header: t('forms.name'),
              ...
              },
              {
                ...
                width: 'w-28',
                align: 'end',
                render: (row) => <RowActionsMenu actions={customerActions(row)} />,
              },
            ]}
          />
        )}
      ...
      <Modal
        ...
        title={editingCustomer ? t('forms.editCustomer') : t('forms.addCustomer')}
      >
        <Formik
          enableReinitialize
          initialValues={{
          ...
          {({ isSubmitting }) => (
            <Form className="space-y-3">
              ...
              <div className="flex flex-col-reverse justify-end gap-2 sm:flex-row">
                <Button type="button" variant="secondary" onClick={() => setCustomerOpen(false)}>
                  {t('common.cancel')}
                </Button>
