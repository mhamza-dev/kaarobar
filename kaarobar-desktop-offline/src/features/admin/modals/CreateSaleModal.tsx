...
export function CreateSaleModal({
  ...
  const [paymentChoice, setPaymentChoice] = useState<'credit' | 'cash' | 'card'>('credit')
  ...
  async function confirmSale() {
    ...
    try {
      ...
      if (canPrint) {
        try {
          await window.api.sales.printReceipt(sale.id)
      ...
      footer={
        <div className="flex w-full flex-col-reverse justify-end gap-2 sm:flex-row">
          ...
          <Button
            type="button"
            loading={submitting}
            disabled={cartItems.length === 0 || hasOverstock || (requireServedBy && !servedByUserId)}
      ...
      <div className="space-y-4">
        ...
        <SearchSelectField
          ...
          onChange={(next) => {
            ...
            if (next) setPaymentChoice('credit')
          }}
        />
